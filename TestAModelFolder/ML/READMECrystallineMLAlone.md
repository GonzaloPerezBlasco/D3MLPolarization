...
It looks in the folder *TestAModelFolder/FileReadingStoring/CrystallineMLDataBase* for all valid experiments. 

<h2>Output: </h2>

1. **Tests**
    Contains all models tested
   
   1.1 **CrystallineAllTestsFolder\_{Complexity}\_{num_augmentations}** 
...
For each experiment in the data base (CrystallineMLDataBase) a folder is created and inside there are all the files that the code has created when training and predicting.
            ...
            1.1.1.1 Missing\_{base\_name}.jpg
This is the important graph. It shows in black the experimental values with their uncertainty, in red the pure ML predictions (without the correction) with the uncertainty bands in a transparent red, in green the final predictions (ML + linear correction) with uncertainty as the green transparent band and in blue the corrected predictions for the time points where experimental values are known
            ...
            1.1.1.2 model_PolarizationD3.keras
The trained file of the model used in this isolated-experiment iteration

            1.1.1.3 PredictedData\_PolarizationD3\_Missing{base\_name}.txt
The predicted values every 1000 seconds (fixed scale)

            1.1.1.4 PredictedPoints\_PolarizationD3\_Missing{base\_name}.txt
The predictions at the same time as the measured values

            1.1.1.5 RawData\_PolarizationD3\_Missing{base\_name}.txt
...
3. **CrystallineLog\_Testing\_ML.txt**
